...
function Conversation({ conversation, active, setActiveConversation}) {
    return (
        <li onClick={() => setActiveConversation(conversation.id)} className={`cursor-pointer transition-all duration-300 ${active ? 'bg-[#abc4ff] ' : 'hover:bg-gray-100 text-gray-700'} border-b border-gray-100 flex gap-2 w-full p-3 rounded-corners `}>
